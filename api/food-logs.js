...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const result = await collection.insertOne(foodLogData);
      const foodLog = await collection.findOne({ _id: result.insertedId });
      ...
      const url = req.url;
      const userId = url.split('/')[2]; // Extract userId from URL path
